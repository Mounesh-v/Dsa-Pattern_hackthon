...
exports.createAttempt = async (req, res, next) => {
  try {
    ...
    // Update analytics
    await updateAnalytics(userId, problem.correctPattern, selectedPattern, isCorrect, timeTaken);
...
async function updateUserStats(userId, pattern, isCorrect, timeTaken) {
  ...
  const totalCorrect = Object.values(user.patternStats).reduce((sum, s) => sum + s.correct, 0);
  user.speedRecords.avgAccuracy = totalCorrect / user.speedRecords.totalAttempts;

  await user.save();
}

// Helper function to update analytics
async function updateAnalytics(userId, correctPattern, selectedPattern, isCorrect, timeTaken) {
  ...
  patternData.avgTime = (patternData.avgTime * (patternData.attempts - 1) + timeTaken) / patternData.attempts;
  ...
  if (isCorrect) {
    ...
    if (analytics.overallStats.currentStreak > analytics.overallStats.bestStreak) {
  ...
  analytics.overallStats.overallAccuracy =
    (analytics.overallStats.totalCorrect / analytics.overallStats.totalAttempts) * 100;

  const totalTime = analytics.patternAccuracy.reduce((sum, data) => sum + data.avgTime * data.attempts, 0);
  analytics.overallStats.avgTime = totalTime / analytics.overallStats.totalAttempts;

  // Update weak and strong patterns
  const patterns = Array.from(analytics.patternAccuracy.entries()).map(([pattern, data]) => ({
    pattern,
